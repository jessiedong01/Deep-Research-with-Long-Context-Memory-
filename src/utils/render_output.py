...
from utils.dataclass import DocumentType, RetrievedDocument
...
def _coerce_documents(items: list[dict | RetrievedDocument]) -> list[RetrievedDocument]:
...
def render_output_markdown(data: dict | str) -> str:
    """
...
def render_output_latex(data: dict | str) -> str:
    if isinstance(data, str):
        data = json.loads(data)
